...
/** check the status of netflow plugin according to scope & if its enable
...
void fetch_all_configurations(char *root_url , char *scope_id)
{
    ...
    fetch_netflow_config(root_url);
    fetch_netflow_monitor(root_url);
}

/**	fetch all associations for netflow plugin according to scope
...
void fetch_associations(char* root_url , char *scope_id)
{
    ...
    while(ass_temp!=NULL)
    {
        ...
        SECMON_DEBUG("fetching associations\n");
...
/** fetch association corresponding to id for netflow plugin 
...
/** store collectors and than fetch collector corresponding to id for netflow plugin 
...
struct Collector* fetch_collector_by_id(char* root_url , char* id, bool is_collector_set)
{
    ...
    add_netflow_destination((char *)obj->ip.dst_addr , obj->port);
    //store_collector_obj(obj);
...
/** fetch policy corresponding to id for netflow plugin 
 *  called after fetching association
 *  @param root_url
 *      contains root url use to fetch details
 *  @param  id
...
/** fetch all rule objects of policy ,  for netflow plugin 
 *  called by policy
 *  @param  p_temp
...
/** fetch rule object of policy ,  for netflow plugin 
 *  @param  p_temp
 *      contains policy contains rule objects ids
 *  @param root_url
 *      contains root url use to fetch details
...
/** fetch rule of policy ,  for netflow plugin 
...
/** fetch collector set corresponding to id for netflow plugin 
 *  called after fetching association
 *  @param root_url
 *      contains root url use to fetch details
 *  @param  id
...
void fetch_scope_id(char *root_url , char *scope_name)
{
    ...
    if(strcmp((char *)plugin.status , DISABLED) == 0)
    {
        update_netflow_status(FALSE);
    }   
    else
    {
        update_netflow_status(TRUE);
    }
...
/**	fetch netflow config
 *  @param root_url
 *      contains root url use to fetch details
 *  @returns void
 *
 */
void fetch_netflow_config(char* root_url)
{
    char* response ,                                /*store response from server*/
        *url = malloc(URL_SIZE*sizeof(char));       /*contain complete url*/
    struct Netflow_Config *obj;

    memset(url,'\0',URL_SIZE);

    /*fetch netflow config*/
    response = get_conf_from_ems(root_url , url , THREETAGS , NETFLOWCONFIG , SCOPEID , plugin.scope->id);
    memset(plugin.url,'\0',strlen((char *)plugin.url));
    strncpy((char *) plugin.url , url , strlen(url));
    ...
    parse_json_response(response , NETFLOWCONFIG);

    obj = plugin.netconfig;

    netflow_config(obj->active_tmout , obj->inactive_tmout , obj->refresh_rate , obj->tm_rate ,  obj->maxflows);
...
/** fetch netflow monitor
 *  @param root_url
 *      contains root url use to fetch details
 *  @returns void
 *
 */
void fetch_netflow_monitor(char* root_url)
{
    char* response ,                                /*store response from server*/
        *url = malloc(URL_SIZE*sizeof(char));       /*contain complete url*/
    struct Netflow_Monitor  *obj;

    memset(url,'\0',URL_SIZE);

    /*fetch monitor*/
    response = get_conf_from_ems(root_url , url , THREETAGS , NETFLOWMONITOR , SCOPEID , plugin.scope->id);
    memset(plugin.url,'\0',strlen((char *)plugin.url));
    strncpy((char *) plugin.url , url , strlen(url));
    ...
    parse_json_response(response , NETFLOWMONITOR);

    obj = plugin.netmonitor;

    add_netflow_monitor_params(obj->match_field ,  obj->collect_field);
...
/**  print all configurations stored for netflow plugin
 *   @returns void
 *
 */
void print_configurations(void)
{
    ...
    /*print config & monitor details*/
    if(plugin.netconfig==NULL || plugin.netmonitor==NULL)
        return;	

    SECMON_DEBUG("config\n");

    SECMON_DEBUG("id=%s\n active tm_out=%d\n",(char *)plugin.netconfig->id,(int)plugin.netconfig->active_tmout);
    SECMON_DEBUG("in active tm_out=%d\n",(int)plugin.netconfig->inactive_tmout);
    SECMON_DEBUG("refresh rate=%d\n",(int)plugin.netconfig->refresh_rate);
    SECMON_DEBUG(" tm_rate=%d\n",(int)plugin.netconfig->tm_rate);
    SECMON_DEBUG("maxflow=%d\n",(int)plugin.netconfig->maxflows);

    SECMON_DEBUG("monitor\n");

    SECMON_DEBUG("id=%s\n match field=%d\n",(char *)plugin.netmonitor->id,(int)plugin.netmonitor->match_field);
    SECMON_DEBUG("collect field=%d\n",(int)plugin.netmonitor->collect_field);
