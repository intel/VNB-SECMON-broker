...
void handle_ems_notifications()
{
    ...
    /*initialize socket fo server*/
    error = initialize_socket(&socket_descriptor);

    /*error in initializing the socket for server*/
    if(error==FAILURE)
    {
        SECMON_CRITICAL("ERROR:RAWFORWARD server can't able to start\n");
        perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
    ...
    /*continuously waiting for receiving responses*/
    for(;;)
    {
        printf( "\n\t....Rawforward Server Is Running....\n");
        ...
        {
            SECMON_CRITICAL("ERROR:RAWFORWARD can't able to accept connection\n");
            perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        ...
        {
          printf("Rawforward: sending ACK\n");
            fflush(stdout);
            if(send(connection_descriptor  , response  ,  strlen(response)  ,  0) < 0)
            {
                SECMON_CRITICAL("ERROR:RAWFORWARD Send failed for GET request\n");
                return;
            }
        ...
        if( send(connection_descriptor  , response  ,  strlen(response)  ,  0) < 0)
        {
            SECMON_CRITICAL("ERROR:RAWFORWARD Send failed\n");
            perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        ...
        {
            SECMON_CRITICAL("ERROR:RAWFORWARD in correct operation\n");
            perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
            fpost(raw_rule_futex);
...
int initialize_socket(int *socket_descriptor)
{
    ...
    *socket_descriptor = socket(AF_INET , SOCK_STREAM , 0);
    ...
    {
        SECMON_CRITICAL("ERROR:RAWFORWARD error in creating socket for server\n");
        perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        return FAILURE;
    }
    ...
    {
        SECMON_CRITICAL("ERROR:RAWFORWARD bind failed\n");
        perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        return FAILURE;
    }
...
void  read_ip_port(char *ip , int *port)
{
    ...
    {
        SECMON_CRITICAL("ERROR:RAWFORWARD Could not open the server config file\n");
        perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        exit(EXIT0);
    }
...
//        print_configurations();
    ...
    {
        SECMON_CRITICAL("ERROR:RAWFORWARD Permission denied! Can't add the requested configuration\n");
        perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        return;
    }
}
...
//        print_configurations();
    ...
    {
        SECMON_CRITICAL("ERROR:RAWFORWARD Permission denied! Can't delete the requested configuration\n");
        perror("Error occur rawforward. Please check /var/log/secmon.log file for error\n");
        return;
    }
}
