...
/** @file
 *	to update configuration according to id if exists 
 *
 */
...
void store_association(char* key , char* value)
{
    ...
    else
    {
        ...
        {
            /*if value contain collector than store it as collector_id and
             *if value contain collectorset than store it as collector_set_id
             */
...
/** store collector or collector inside collector set with details if not exists
 *  @param key
 *      contains key
 *  @param  value
 *      contains value according to key
 *  @param  is_collector_set
 *      contains flag to denote that details provided is for collector or collector_set
 *  @returns void
 *
 */
...
void store_collector(char* key , char* value, bool is_collector_set)
{
    ...
    if(strcmp(key , UUID) == 0)
    {
        ...
        if(head==NULL)
        {
            //SECMON_DEBUG("store_collector: head is null\n");
            if(is_collector_set)
            {
                collector_set = find_last_collector_set(plugin.collector_set);
                collector_set->collectors = Collec;
                //SECMON_DEBUG("plugin.collector_set = %p\n",collector_set);
            }
            else
            {
                plugin.collector = Collec;
                //SECMON_DEBUG("store_collector: Collector\n");
            }
        }
        else
        {
            //SECMON_DEBUG("store_collector: head is not null\n");
            if(!is_collector_set)
            {
                ...
                if(loc!=NULL)
                {
                    ...
                    //SECMON_DEBUG("collector id already exists\n");
                    if(Collec!=NULL)
                    {
                        free(Collec);
                        Collec = NULL;
    ...
    else if(strcmp(key, WEIGHT) == 0)
    {
        ...
        {
            //SECMON_DEBUG("store_collector: Searching last collector\n");
 ...
 *  @param cobj
 *      collector object in which to store collector details
 *  @param collector
 *      collector from which the details should be copied
 *  @return void
 *
 */
void copy_collector(struct Collector_object *cobj, struct Collector *collector)
{
    ...
    {
        //SECMON_DEBUG("first collector set\n");
    ...
    {
        //SECMON_DEBUG("second collector set\n");
...
void store_collector_set_obj(struct Collector *collector, bool is_first_collector)
{
    ...
    //SECMON_DEBUG("inside store_collector_set_obj\n");
    if(head == NULL)
    {
        //SECMON_DEBUG("Collector object is NULL\n");
        ...
        collector_set = find_last_collector_set(plugin.collector_set);
        //SECMON_DEBUG("collector_set->id = %s\n",collector_set->id);
 ...
 */
 void store_collector_obj(struct Collector *collec)
 {
    ...
    //SECMON_DEBUG("inside store_collector_obj\n");
...
{
    //SECMON_DEBUG("store_collector_set\n");
...
void fetch_update_collector(char* id)
{
    ...
    while(ass_temp!=NULL)
    {
        ...
        if(strcmp((char *)ass_temp->collector_id , id) == 0)
        {
            ...
            //SECMON_DEBUG("fetch_update_collector\n");
        ...
        //SECMON_DEBUG("plugin.collector_set id = %s, cset_id %s\n",plugin.collector_set->id, ass_temp->collector_set_id);
        collector_set = collector_set_node_of_id(plugin.collector_set, (char *)ass_temp->collector_set_id);
        //SECMON_DEBUG("collector_set = %p\n",collector_set);
        if (collector_set)
        {
...
{   
    ...
    {
        memset(ass_id,'\0',MAX_ID_LEN);
        //SECMON_DEBUG("fetch_update_collectorset: searching associations\n");
        if(strcmp((char *)ass_temp->collector_set_id,"") != 0)
        {
            if(strcmp((char *)ass_temp->collector_set_id , id) == 0)
            {
                //SECMON_DEBUG("fetch_update_collectorset: association found\n");
...
/** update netflow configurations by deleting and add new
 *  @returns void
 *
 */
void update_netflow_config()
{
    ...
    fetch_netflow_config((char *)plugin.root_url);
}

/** update netflow monitor configurations by deleting and add new
 *  @returns void
 *
 */
void update_netflow_monitor()
{
    ...
    fetch_netflow_monitor((char *)plugin.root_url);

}

/** store netflow configurations
 *  @param key
 *      contains key
 *  @param  value
 *      contains value according to key
 ...
 *
 */
void store_netflow_config(char *key , char *value)
{
    struct Netflow_Config *config;

    /*key is id*/
    if(strcmp(key , UUID) == 0)
    {
        config =  malloc(sizeof( struct Netflow_Config));
        memset(config,'\0',sizeof( struct Netflow_Config));

        strncpy((char *) config->id , value , strlen(value));

        plugin.netconfig = config;
    }
    /*store config details*/
    else
    {
        config = plugin.netconfig;

        if(strcmp(key , ACTIVE_TIMEOUT) == 0)
        {
            config->active_tmout=(uint8_t)atoi(value);
        }

        if(strcmp(key , INACTIVE_TIMEOUT) == 0)
        {
            config->inactive_tmout=(uint8_t)atoi(value);
        }

        if(strcmp(key , REFRESH_RATE) == 0)
        {
            config->refresh_rate=(uint8_t)atoi(value);
        }

        if(strcmp(key , TIMEOUT_RATE) == 0)
        {
            config->tm_rate=(uint8_t)atoi(value);
        }

        if(strcmp(key , MAXFLOWS) == 0)
        {
            config->maxflows=(uint8_t)atoi(value);
        }
    }
}


/** store netflow monitor configurations
 *  @param key
 *      contains key
 *  @param  value
 *      contains value according to key
 ...
 *
 */
void store_netflow_monitor(char *key , char *value)
{
    struct Netflow_Monitor *monitor;

    /*key is id*/
    if(strcmp(key , UUID) == 0)
    {
        monitor =  malloc(sizeof( struct Netflow_Monitor));
        memset(monitor,'\0',sizeof( struct Netflow_Monitor));

        strncpy((char *) monitor->id , value , strlen(value));

        plugin.netmonitor = monitor;

    }
    else
    {
        monitor = plugin.netmonitor;

        if(strcmp(key , MATCH_FIELD) == 0)
        {
            monitor->match_field = generate_match_field(value);
        }

        if(strcmp(key , COLLECT_FIELD) == 0)
        {
            monitor->collect_field = generate_collect_field(value);
        }
    }
}
