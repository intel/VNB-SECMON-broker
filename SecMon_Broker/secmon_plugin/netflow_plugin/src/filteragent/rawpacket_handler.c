...
struct rule_hash *netflow_rule_hash_table[HASH_TABLE_SIZE];

/**  initialize netflow rule hash table with null entries
 *   @returns void
 *
 */
void initialize_netflow_rule_hash_table(void)
{
    int i;
...
/** add hash entry to hash table for netflow plugin
 * @param 
...
void add_hash_entry(uint32_t hash_code,  bool action ,  uint32_t truncate_to_size)
{
    ...
    if(netflow_rule_hash_table[hash_code] == NULL)
    {
        netflow_rule_hash_table[hash_code]  =  hash_entry;
        netflow_rule_hash_table[hash_code]->action  =  action;
        //netflow_rule_hash_table[hash_code]->encap_protocol  =  encap_protocol;
		netflow_rule_hash_table[hash_code]->truncate_to_size  =  truncate_to_size;
    }
    else
    {
        struct rule_hash *rhash  =  netflow_rule_hash_table[hash_code];
        while(rhash != NULL)
        {
            rhash  =  rhash->next;
        }
...
{
    if(netflow_rule_hash_table[hash] != NULL)
    {
        return (netflow_rule_hash_table[hash]->action == FORWARD);
    }
    return FALSE;
