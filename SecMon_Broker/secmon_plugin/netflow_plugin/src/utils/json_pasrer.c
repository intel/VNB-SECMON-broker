...
void get_json_value(json_object *jobj , char* obj , char* key)
{
    ...
    /*store netflow config*/
    if(strcmp(obj , NETFLOWCONFIG) == 0)
    {
        store_netflow_config(key , res1);
        return;
    }

    /*store netflow monitor*/
    if(strcmp(obj , NETFLOWMONITOR) == 0)
    {
        store_netflow_monitor(key , res1);
        return;
    }
