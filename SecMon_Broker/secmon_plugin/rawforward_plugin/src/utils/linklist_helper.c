...
void find_collector_by_policy(char *p_id , char *collector_id)
{
    ...
    while(temp!=NULL)
    {
        if(strcmp((char*)temp->policy_id , p_id) == 0)
        {
            ...
            /*if(strcmp(collector_id, "") == 0)
            {
                collector_set = plugin.collector_set;
                while(collector_set != NULL)
                {
                    
                    collector_set = collector_set->next;
                }
                strncpy(collector_id,(char *)temp->collector_set_id , strlen((char *)temp->collector_set_id));   
                printf("collector_id = %s\n",collector_id);
            }*/
            return;
        }
