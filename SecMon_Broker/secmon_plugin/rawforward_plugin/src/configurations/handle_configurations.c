...
void delete_association(char *id)
{
    ...
    while(node!=NULL)
    {
        ...
        if(strcmp((char *)node->id , id) == 0)
        {
            ...
            {
                col_set = COLSET;
                SECMON_DEBUG("deleting collector set\n");
...
void delete_collector_set(char *id)
{
    ...
    while(node!=NULL)
    {
        ...
        if(strcmp((char *)node->id , id) == 0)
        {
            /*delete collector object*/
            col_set = COLSET;
            // delete_collector_obj((char *)(node->collectors)->id);
...
/** Add new association using scope
 * and store the details from association
 * @param key
 *     contains key
 * @param value
