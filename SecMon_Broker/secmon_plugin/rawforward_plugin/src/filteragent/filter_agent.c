...
static int count_apply_filters;
#endif
int tool_count = 0;
 ...
 * @returns 
 *     list of tools to send the packet to. 
 *
 */
void apply_filters(struct Tuple *tuple , uint32_t hash , bool *found)
{
    // taking reference of global collectors list
    ...
    if(unlikely(temp == NULL))
    {
        ...
        {
            SECMON_DEBUG("No configurations found for rawforward plugin\n");
    ...
    if(rule_matched == TRUE)
    {
        *found  =  TRUE;
        ...
        hash_table[hash]->last_seen  =  get_jiffies();
                                                ...
                                                bool in_collector_set, uint8_t algo)
{
    ...
    else
    {
        ...
        SECMON_DEBUG("id in copy_collector_details= %s\n",collector_details->id);
...
{
    return hash_table[hash_code]->collectors;
}

/** add tools list to collectors object
 * in the hash entry with hash value hash_code.
 * @param hash_code 
 *     hash code for the tuple. 
 * @param tools 
...
inline void add_tools_to_hash(uint32_t hash_code ,  struct Collector_object *tools)
{
    ...
    struct Collector_object *cobj_temp1 = hash_table[hash_code]->collectors;
...
/**
 * Loops over the collectors provided and sends the packet to each collector.  
 * @param pkt 
 *     pointer to the packet data
 * @param hash
 *     hash index of the tuple 
 * @param packet_len
 *     length of the packet. 
 * @return
 *     void 	
 */
void send_to_tools(char *pkt ,  unsigned int hash_code ,  int packet_len)
{
  // int pkt_len  =  packet_len;
#ifdef SECMON_DEBUG_LOG
  unsigned long int packets_sent;
#endif

  /* If packet to be sent need to be truncated, get the truncated size*/
  int truncate_len  =  hash_table[hash_code]->truncate_to_size;
  if(truncate_len)
  {     
    packet_len  =  (truncate_len < packet_len)? truncate_len: packet_len;
  }

  /* update the last_seen with present time. */
  hash_table[hash_code]->last_seen  =  get_jiffies();

  /*fetch the details of the tools from the hash entry of the tuple. */
  struct Collector_object *tools  =  fetch_tools(hash_code);

  while(tools != NULL)
  {
    if(tools->encap_protocol == UDP)
    {
      sendto(tools->socket, pkt, packet_len /* pkt_len */, 0 ,   
          (struct sockaddr *)&tools->server_address ,  
          tools->server_length);
    }
    else
    {
...
void decrement_session(struct Collector_object *collec_obj)
{
  /* To remove multiple collector corresponding to single rule */
    ...
    while (collec_obj != NULL) 
    {
      ...
      SECMON_DEBUG("Iterating Collector object inside decrement session id: %s \n", collec_obj->collector_id);
